...
namespace RestClient.Net
{
    public class ProtobufSerializationAdapter : ISerializationAdapter
    {
        ...
        public TResponseBody Deserialize<TResponseBody>(byte[] responseData, IHeadersCollection? responseHeaders)
        {
            ...
            var parserProperty = messageType.GetProperty("Parser") ?? throw new InvalidOperationException("Could not get Parser");
            var parser = parserProperty.GetValue(parserProperty);
            var parseFromMethod = parserProperty.PropertyType.GetMethod("ParseFrom", new Type[] { typeof(byte[]) }) ?? throw new InvalidOperationException("Could not get ParseFrom method");
